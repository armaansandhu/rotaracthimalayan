...
import 'package:rotaract_app/core/ui/my_notifications/checkbox_reactive_tile.dart';
...
class _NotificationListState extends State<NotificationList> {
  ...
  //Sort list of events as "checked >> unchecked"
  sortList(List<DocumentSnapshot> docs, List<String> subscription) {
    docs.forEach((doc) {
      if (subscription.contains(doc['id']) == true) tmp.add(doc);
    });
    docs.forEach((doc) {
      if (subscription.contains(doc['id']) == false) tmp.add(doc);
    });
  }
        ...
        body: FutureBuilder(
          ...
          builder: (BuildContext context, AsyncSnapshot snapshot) {
            if (snapshot.connectionState == ConnectionState.done) {
              ...
              return ListView(
                children: tmp
                    .map((doc) => EventCheckBoxTile(
                        doc: doc, subscription: subscription, id: widget.user))
                    .toList(),
              );
            } else
              return Center(
